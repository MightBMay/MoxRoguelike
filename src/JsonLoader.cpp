...
const std::string GameData::getPlayerClassNameFromIndex(const int& index) {
	return (*s_player_classes)["IndexToClass"][std::to_string(index)];
}

const json& GameData::getPlayerClassFromIndex(const int& index) {
	return getPlayerClass(
		getPlayerClassNameFromIndex(index)
	);
}
