...
GameObject::GameObject(const std::string& path, const sf::IntRect& rect) {
	...
	renderable = std::make_shared<Renderable>(sprite, sprite, nullptr);
};
...
void GameObject::setSprite(const std::string& path, const sf::IntRect& rect) {
	...
	renderable->transform = sprite;


}
void GameObject::setSprite(const std::shared_ptr<sf::Texture>& texture, const sf::IntRect& rect) {
...
void GameObjectManager::renderAll(sf::RenderTarget& target) {
	...
	// instead of doing proper bounds checks to fully prevent popping, we only cull things farther than 256 px off screen.
	static constexpr sf::Vector2f sizeOffset = { 256,256 };
	sf::Vector2f viewSize= playerView->getSize() + sizeOffset;
	sf::Vector2f viewCenter = playerView->getCenter();
	const sf::FloatRect& playerViewRect{
		{
	...
	for (auto& [layer, renderables] : renderLayers_) {
		...
		for (auto& renderable : renderables) {
			if (renderable && renderable->enabled) {
				...
				std::shared_ptr<sf::Drawable> drawable = renderable->drawable.lock();
				std::shared_ptr<sf::Transformable> transform = renderable->transform.lock();
				if (!drawable)
					continue;


				// simple culling.
				if (!layerIsUI && transform) // if current view isn't UI, and a transform is set for the renderable, 
					if (!playerViewRect.contains(transform->getPosition())) // if not contained in the frustrum 
