...
public:
    ...
    Weapon(std::shared_ptr<WeaponStats> stats):
        stats(stats){}

    virtual void Fire() {
        //sf::Vector2f direction = getMouseWorldPos(window, window.getDefaultView()) - Player->getPosition(); direction.normalized()
        projPool->make<Projectile>(stats->speed, sf::Vector2f(1, 1).normalized(), stats->range, stats->projRadius);
        fireTimer = 1 / stats->fireRate;
    }
    virtual void init() { projPool = &ProjectilePool::getInstance(); }
